...
class Validate(testcase.TestCase):
    ...
    def run(self, **kwargs):
        ...
        if showTimeStamps:
            resj["timeStamps"] = {}
            resj["timeStamps"]["startTime"] = f"{time2datetime(self.start_time)}"
            resj["timeStamps"]["stopTime"] = f"{time2datetime(self.stop_time)}"

        #print(json.dumps(endresj, indent=2))

        os.makedirs(self.res_dir, exist_ok=True)
        with open('{}/dump.txt'.format(self.res_dir), 'w+') as report:
            json.dump(endresj, report, indent=4, sort_keys=True)
        self.result = 1