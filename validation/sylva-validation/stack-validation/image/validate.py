...
def checkEmptyNamespace():
    ...
    for i in r.items:
        if i.metadata.namespace == NS:
            resj["error"] = f"There are already pods running in namespace {NS}."
            return False
    return True
            ...
            if p.metadata.namespace == NS and p.metadata.name.startswith(f"test-{CPUPOWER}-"):
                ...
                for l in log.split("\n"):
                    if "cpussamehwfreq=" in l:
                        ...
                        debug_cpupower = l
                res_cpupower = shf
            if p.metadata.namespace == NS and p.metadata.name.startswith(f"test-{KERNELRT}-"):
                node_with_kernelrt_pod = True
                log = v1.read_namespaced_pod_log(namespace=NS, name=p.metadata.name)
            ...
            if p.metadata.namespace == NS and p.metadata.name.startswith(f"test-{TUNEDRT}-") and p.status.phase == "Running":
                ...
                for l in log.split("\n"):
                    ...
                    if "tunedlogstatictuning=" in l:
                        debug_tunedrt = l
                res_cpupower = shf
        if node_with_cpupower_pod and node_with_kernelrt_pod and node_with_tunedrt_pod:
...
def validateTSN():
    ...
    for ln in NODES:
        ...
        if node_with_ptphwclock_pod:
            ...
            cwn["pass"] = "false"
            cwn["error"] = f"Doesn\'t find pod test-{PTPHWCLOCK}"
...
if showTimeStamps == "true":
    resj["timeStamps"] = {}
    resj["timeStamps"]["startTime"] = f"{startTime}"
    stopTime = time.time()
    resj["timeStamps"]["stopTime"] = f"{stopTime}"
